...
from backend.inpaint.video.model.recurrent_flow_completion import (
...
class VideoInpaint:
    def __init__(self, sub_video_length=config.PROPAINTER_MAX_LOAD_NUM, use_fp16=True):
        ...
        self.use_fp16 = use_fp16 and (self.device != torch.device("cpu"))
    ...
    def init_raft_model(self):
        ...
        return RAFT_bi(
            os.path.join(config.VIDEO_INPAINT_MODEL_PATH, "raft-things.pth"),
            self.device,
        )
    ...
    def init_fix_flow_model(self):
        fix_flow_complete_model = RecurrentFlowCompleteNet(
            os.path.join(
                config.VIDEO_INPAINT_MODEL_PATH, "recurrent_flow_completion.pth"
            )
        )
        ...
        return (
            InpaintGenerator(
                model_path=os.path.join(
                    config.VIDEO_INPAINT_MODEL_PATH, "ProPainter.pth"
                )
            )
            .to(self.device)
            .eval()
                ...
                for f in range(0, video_length, short_clip_len):
                    ...
                    if f == 0:
                        flows_f, flows_b = self.fix_raft(
                            frames[:, f:end_f], iters=self.raft_iter
                        )
                    else:
                        flows_f, flows_b = self.fix_raft(
                            frames[:, f - 1 : end_f], iters=self.raft_iter
                        )
...
if __name__ == "__main__":
    ...
    frames = read_frames(
        "/home/yao/Documents/Project/video-subtitle-remover/local_test/test1.mp4"
    )
    mask = cv2.imread(
        "/home/yao/Documents/Project/video-subtitle-remover/local_test/test1_mask.png"
    )
    ...
    video_writer = cv2.VideoWriter(
        video_out_path, cv2.VideoWriter_fourcc(*"mp4v"), 24, (640, 360)
    )
