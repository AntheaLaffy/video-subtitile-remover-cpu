...
class STTNInpaint:
    def __init__(self):
        ...
        self.model.load_state_dict(
            torch.load(config.STTN_MODEL_PATH, map_location="cpu")["netG"]
        )
        # 3. # 将模型设置为评估模式
        self.model.eval()
        # 模型输入用的宽和高
    ...
    def get_inpaint_area_by_mask(H, h, mask):
        ...
        while from_H != 0:
            ...
            # 检查当前段落是否包含遮罩像素
            if (
                not np.all(mask[from_H:to_H, :] == 0)
                and np.sum(mask[from_H:to_H, :]) > 10
            ):
        ...
        try:
            ...
            for i in range(rec_time):
                ...
                if valid_frames_count == 0:
                    print(
                        f"Warning: No valid frames found in range {start_f+1}-{end_f}. Skipping this segment."
                    )
                    continue
                ...
                if inpaint_area and valid_frames_count > 0:
                    for j in range(valid_frames_count):
                        ...
                        if input_sub_remover is not None:
                            ...
                            if (
                                original_frame is not None
                                and input_sub_remover.gui_mode
                            ):
...
if __name__ == "__main__":
    ...
    sttn_video_inpaint = STTNVideoInpaint(
        video_path, mask_path, clip_gap=config.STTN_MAX_LOAD_NUM
    )
    sttn_video_inpaint()
